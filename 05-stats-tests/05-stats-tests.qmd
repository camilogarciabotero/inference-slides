...
t.test(body_mass_g ~ sex, adelie_penguins, var.equal = TRUE, paired = TRUE)
```

------------------------------------------------------------------------

Another alternative is to use the `statExpressions` library:
...
ggplot(adelie_penguins, aes(x = sex, y = body_mass_g, fill = sex)) +
  ...
  labs(
    y = "Body mass (g)",
    x = "",
    subtitle = parse(text = adelie_ttest_table$expression),
  )
```